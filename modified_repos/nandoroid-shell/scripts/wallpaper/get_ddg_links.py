...
LOG_DIR = os.path.join(CACHE_DIR, "quickshell/nandoroid/logs")
RUN_DIR = os.environ.get("XDG_RUNTIME_DIR", "/tmp")
RUN_WP = os.path.join(RUN_DIR, "quickshell/nandoroid/wallpaper_picker")

LOG_FILE = os.path.join(LOG_DIR, "ddg_python_scraper.log")
CONTROL_FILE = os.path.join(RUN_WP, "ddg_search_control")
    ...
    for page in range(5): 
        ...
        try:
            ...
            for res in results:
                ...
                if (width == 0 or width >= 600) and (height == 0 or height >= 400):
