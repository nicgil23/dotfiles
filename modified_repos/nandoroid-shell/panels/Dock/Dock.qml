    ...
    Variants {
        ...
        delegate: Scope {
            ...
            PanelWindow {
                ...
                readonly property real dockScale: (Config.ready && Config.options.dock ? Config.options.dock.scale : 1) * Appearance.effectiveScale
                ...
                readonly property bool hasActiveWindows: {
                    if (!Config.ready || !HyprlandData.activeWorkspace)
                        return false;
                    ...
                    for (let i = 0; i < windows.length; i++) {
                        const w = windows[i];
                        if (w.monitor === screenScope.monitorIndex && !w.floating && w.mapped && !w.hidden)
                            return true;
                ...
                property bool reveal: {
                    ...
                    if (root.pinned || GlobalStates.dockMenuOpen || dockPreview.visible || dockPreview.hovered || dockApps.buttonHovered || dockMouseArea.containsMouse)
                        return true;

                    // Auto-hide logic
                    if (Config.options.dock.autoHide) {
                        if (Config.options.dock.autoHideMode === 1)
                            return false;
 // Always hide mode
                ...
                exclusiveZone: {
                    if (!Config.ready || !visible)
                        return 0;

                    const scale = (Config.options.dock.scale ?? 1) * Appearance.effectiveScale;
                ...
                color: "transparent"
                visible: {
                    if (!Config.ready || GlobalStates.screenLocked || !Config.options.dock.enable)
                        return false;

                    // If 'Show Only In Desktop' is ON, only show if no active windows on this monitor
                    if (Config.options.dock.showOnlyInDesktop) {
                        if (GlobalStates.launcherOpen || GlobalStates.dockMenuOpen || root.pinned)
                            return true;
                ...
                MouseArea {
                    ...
                    height: {
                        ...
                        if (dockPreview.visible || dockPreview.hovered)
                            return parent.height;

                        return dockWindow.reveal ? parent.height : 10 * Appearance.effectiveScale;
                    }
                        ...
                        Item {
                            ...
                            Item {
                                ...
                                Rectangle {
                                    ...
                                    opacity: dockWindow.bgStyle === 0 ? 0 : 1
                                    // MD3 Outline Style
                                ...
                                Item {
                                    ...
                                    RowLayout {
                                        ...
                                        DockApps {
                                            ...
                                            buttonPadding: 6 * Appearance.effectiveScale
                                            spacing: 8 * Appearance.effectiveScale
                                            height: visualContainer.height
                                        ...
                                        DockButton {
                                            ...
                                            dockTopInset: 6 * Appearance.effectiveScale
                                            dockBottomInset: 6 * Appearance.effectiveScale
                                            ...
                                            background: Item {
                                                ...
                                                Rectangle {
                                                    ...
                                                    color: overviewButton.baseColor
                                                ...
                                                MaterialShape {
                                                    ...
                                                    color: overviewButton.down ? Appearance.colors.colPrimary : Appearance.colors.colPrimaryContainer
                                                }
                                            ...
                                            contentItem: Item {
                                                anchors.fill: parent
                                                scale: overviewButton.down ? 0.92 : (overviewButton.hovered ? 1.05 : 1)
                                                ...
                                                MaterialSymbol {
                                                    ...
                                                    anchors.centerIn: parent
                                                    text: "grid_view"
                                                    iconSize: (Config.ready && Config.options.dock.monochromeIcons ? 22 : 26) * Appearance.effectiveScale
                                                    color: overviewButton.toggled ? Appearance.colors.colPrimary : Appearance.colors.colOnLayer0
                                                    visible: !(Config.ready && Config.options.dock.monochromeIcons)
                                                }

                                                ColorOverlay {
                                                    anchors.fill: overviewIcon
                                                    source: overviewIcon
                                        ...
                                        DockButton {
                                            ...
                                            contentItem: Item {
                                                anchors.fill: parent
                                                scale: launcherButton.down ? 0.92 : (launcherButton.hovered ? 1.05 : 1)
                                                ...
                                                MaterialSymbol {
                                                    ...
                                                    anchors.centerIn: parent
                                                    text: "apps"
                                                    iconSize: (Config.ready && Config.options.dock.monochromeIcons ? 24 : 28) * Appearance.effectiveScale
                                                    color: launcherButton.toggled ? Appearance.colors.colPrimary : Appearance.colors.colOnLayer0
                                                    visible: !(Config.ready && Config.options.dock.monochromeIcons)
                                                }
