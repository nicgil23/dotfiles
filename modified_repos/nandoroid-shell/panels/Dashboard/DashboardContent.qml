...
Item {
    ...
    property int currentTab: GlobalStates.dashboardActiveTab
    onCurrentTabChanged: {
    ...
    Connections {
        ...
        function onDashboardOpenChanged() {
            if (GlobalStates.dashboardOpen) {
                currentTab = GlobalStates.dashboardActiveTab
                tabHighlight.reset()
                root.forceActiveFocus()
            } else {
                GlobalStates.dashboardActiveTab = 0
            }
        }
    }
        ...
        Rectangle {
            ...
            Item {
                ...
                Loader {
                    ...
                    Behavior on opacity { NumberAnimation { duration: 200; easing.type: Easing.OutQuart } }
                    sourceComponent: DashTimer { width: contentArea.width; height: contentArea.height }
                }
                ...
                Loader {
                    ...
                    onVisibleChanged: {
                        if (visible && item && !item.loading && (item.errorMsg !== "" || !item.profile)) {
                            item.fetch()
                        }
                    }
                }
